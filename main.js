...
var category= data.events.map(evento =>evento.category)
...
function filtroCombinado(){//Creo la funcion para filtrar el checkbox y el search juntos, esta no necesita de parametros porque va trabjar con parametro ya establecidos
    ...
    if(escuchadorCheck.length>0 && searchEscuchador !== ""){
        escuchadorCheck.map(checking=>{contenedorFiltro.push(...data.events.filter(searching=>searching.name.toLowerCase().startsWith(searchEscuchador.toLowerCase().trim())
        && searching.category == checking))})
    }else if(escuchadorCheck.length>0 && searchEscuchador===""){
        escuchadorCheck.map(checking=>{contenedorFiltro.push(...data.events.filter(searching=>searching.category===checking))})
    }else if(searchEscuchador !=="" && escuchadorCheck.length===0){
        contenedorFiltro.push(...data.events.filter(searching=>searching.name.toLowerCase().startsWith(searchEscuchador.toLowerCase().trim())))
    }else{
        contenedorFiltro.push(...data.events)
    }
...
function displayCardEvents(array, template){
    ...
    displayCardEvents(arreglo, impresion)
