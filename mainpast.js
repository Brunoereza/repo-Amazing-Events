...
var pastEvents= data.events.filter(past=> fechaActual>past.date)
...
var category= pastEvents.map(evento =>evento.category)
...
function filtroCombinado(){//Creo la funcion para filtrar el checkbox y el search juntos, esta no necesita de parametros porque va trabjar con parametro ya establecidos
    ...
    if(escuchadorCheck.length>0 && searchEscuchador !== ""){
        escuchadorCheck.map(checking=>{contenedorFiltro.push(...pastEvents.filter(searching=>searching.name.toLowerCase().startsWith(searchEscuchador.toLowerCase().trim())
        && searching.category == checking))})
    }else if(escuchadorCheck.length>0 && searchEscuchador===""){
        escuchadorCheck.map(checking=>{contenedorFiltro.push(...pastEvents.filter(searching=>searching.category===checking))})
    }else if(searchEscuchador !=="" && escuchadorCheck.length===0){
        contenedorFiltro.push(...pastEvents.filter(searching=>searching.name.toLowerCase().startsWith(searchEscuchador.toLowerCase().trim())))
    }else{
        contenedorFiltro.push(...pastEvents)
    }
...
function displayCardEvents(array, template){
    template.innerHTML = "";
        ...
        if(template==="")
        {template=`<h3 class="text-light d-flex justify-content-center m-5">No results found</h3>`
            document.querySelector('.mainCards').innerHTML=template
        }
